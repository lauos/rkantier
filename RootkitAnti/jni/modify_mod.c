...
int load_and_modify(const char* path, char* true_magic, unsigned long true_checksum)
{
	FILE* fd;
	unsigned int i = 0;
	unsigned int j = 0;
...
/*		EGIS_DEBUG("Section[%u] : 0x%08x\n", i, shdr->sh_type);
	...
	for(i = 0; i < ehdr->e_shnum; i++, shdr++)
	{
		...
		if(!strcmp(&(sh_strtab[shdr->sh_name]), ".modinfo"))
		{
			...
			memcpy(modmagic, true_magic, strlen(true_magic) + 1);
		}
	}

	fd = fopen(oriso,"wb");

	if(fwrite(soPtr, 1, soSize, fd) != soSize)
    {
        EGIS_ERROR("Write file error\n");
        fclose(fd);
        exit(1);
    }
